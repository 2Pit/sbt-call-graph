...
- Outputs JSON (MCP + CLI), or Graphviz DOT (CLI `--format dot`)
...
Results are written to `target/call-graph/N.{json,dot}`. The file path is printed to stdout.
...
- [`graphVia.json`](examples/graphVia.json) — neighbourhood of `CallGraphState.getOrLoad` as JSON with `readHints` for efficient source reading
...
- [`graphPath.dot`](examples/graphPath.dot) — the same path as Graphviz DOT (`--format dot`); render with `dot -Tsvg`
